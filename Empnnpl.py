...
def extract_labels(graph_tensor):
    index = graph_tensor.context['event_energy']
    index = log10(index)
    return graph_tensor,index


##############################
train_ds_provider = runner.TFRecordDatasetProvider(file_pattern="/n/holyscratch01/arguelles_delgado_lab/Everyone/tzhu/SAM_4nnIDmetrics_1_70")
valid_ds_provider = runner.TFRecordDatasetProvider(file_pattern="/n/holyscratch01/arguelles_delgado_lab/Everyone/tzhu/SAM_4nnIDmetrics_100_105")
...
#########
def MAPE_E(y_actual,y_pred):
    mape = tf.keras.losses.MeanAbsolutePercentageError()
    loss = mape(10**y_actual,10**y_pred)
    return loss

metrics = [MAPE_E,'mae','mse']
########
datee = 'knn1012'
...
simple_ts_model.compile(tf.keras.optimizers.Adam(),loss='mse',metrics=metrics)
...
simple_ts_model2.compile(tf.keras.optimizers.Adam(),loss='mse',metrics=metrics)
...
simple_ts_model3.compile(tf.keras.optimizers.Adam(),loss='mse',metrics=metrics)
...
simple_ts_model4.compile(tf.keras.optimizers.Adam(),loss='mse',metrics=metrics)
...
simple_ts_model5.compile(tf.keras.optimizers.Adam(),loss='mse',metrics=metrics)
