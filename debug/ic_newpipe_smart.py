...
def makeGT(id,counter):
    ...
    for i in range(len(suhits)):
        ...
        nmetric = (hits[np.int32(target),0]-hits[np.int32(source),0])*0.3*(1./1000.)
    ...
    stbinding = knndatap.knndatap(suhits.flatten(),len(suhits))   
    if bool(stbinding)==False:
        return 0
       
    #print(tmp)
    source_u = sortinver[np.int32(stbinding[1::3])]
    target_u = sortinver[np.int32(stbinding[2::3])]

    metric_u = np.array(stbinding[3::3])/1000.

    hit_sources = np.concatenate((source_t,source_u,suhits[:,-1]))
    hit_targets = np.concatenate((target_t,target_u,suhits[:,-1]))
    u_m = np.concatenate(([metric_u],[np.zeros(len(metric_u))]),axis = 0).T
    t_m = np.concatenate(([metric_t],[np.ones(len(metric_t))]),axis = 0).T
    s_m = np.concatenate(([np.zeros(len(suhits))],[np.zeros(len(suhits))]),axis = 0).T
    metrics = np.concatenate((t_m,u_m,s_m))
    
    hits = hits[:,:4]
    hits = hits*[0.3,1,1,1]*(1./1000.)
    #print(tmp)
    ...
    return graphtensor, np.concatenate((fcc,[fenergy,fzenith,fazimuth,hit_num,fbjorkeny,smt12[0],smt8[0]]),axis = 0)



##############################
counter =0
filename = 'ICnewtrain_smart'
