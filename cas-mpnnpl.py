...
train_ds_provider = runner.TFRecordDatasetProvider(file_pattern=f"/n/holyscratch01/arguelles_delgado_lab/Everyone/tzhu/2nnE-ortho120SAM_1_50")

valid_ds_provider = runner.TFRecordDatasetProvider(file_pattern=f"/n/holyscratch01/arguelles_delgado_lab/Everyone/tzhu/2nnE-ortho120SAM_4650_5050")
...
def complex_mpnn_model(hidden_size_emb,hidden_gcn1,hidden_gcn2,hidden_gcn3,hidden_gcn4,graph_tensor_spec):
    ...
    graph_pool = tf.concat([graph_pool_1,graph_pool_2],axis = 1)
...
################################
simple_ts_model =  complex_mpnn_model(hidden_size_emb=144,hidden_gcn1=72,hidden_gcn2=36,hidden_gcn3=36,hidden_gcn4=14,graph_tensor_spec=gtspec)
...
def mae_loss(y_actual,y_pred):
    ...
    loss = loss1*100 
    return loss
def p_loss(y_actual,y_pred):
    mae = tf.keras.losses.MeanAbsoluteError()
        
    loss2 = mae(y_actual,y_pred)
    loss=loss2
    if(y_actual<2)&(y_pred>y_actual):
        loss = loss2*100
    if(y_actual>2)&(y_pred<y_actual):
        loss = loss2*100
...
simple_ts_model2.compile(tf.keras.optimizers.Adam(),loss=mae_loss,metrics=task.metrics())
