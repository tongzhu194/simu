...
def complex_mpnn_model(hidden_size_emb,hidden_gcn1,hidden_gcn2,hidden_gcn3,hidden_gcn4,graph_tensor_spec):
    ...
    graph_pool_1 = tf.keras.layers.Dense(units=12,activation='tanh'  )(graph_pool_1)
    graph_pool_1 = tf.keras.layers.Dense(units=8,activation='tanh' )(graph_pool_1)
    graph_pool_1 = tf.keras.layers.Dense(units=6,activation='tanh',name ='embed_pool_1' )(graph_pool_1)
    ...
    graph_pool_2 = tf.keras.layers.Dense(units=12,activation='tanh' )(graph_pool_2)
    graph_pool_2 = tf.keras.layers.Dense(units=8,activation='tanh' )(graph_pool_2)
    graph_pool_2 = tf.keras.layers.Dense(units=6,activation='tanh' ,name ='embed_pool_2')(graph_pool_2)
    ...
    graph_pool_3 = tf.keras.layers.Dense(units=12,activation='tanh')(graph_pool_3)
    graph_pool_3 = tf.keras.layers.Dense(units=8,activation='tanh' )(graph_pool_3)
    graph_pool_3 = tf.keras.layers.Dense(units=6,activation='tanh',name ='embed_pool_3' )(graph_pool_3)
    ...
    graph_pool_4 = tf.keras.layers.Dense(units=12,activation='tanh' )(graph_pool_4)
    graph_pool_4 = tf.keras.layers.Dense(units=8,activation='tanh' )(graph_pool_4)
    graph_pool_4 = tf.keras.layers.Dense(units=6,activation='tanh',name ='embed_pool_4' )(graph_pool_4)
    
    graph_pool = tf.concat([graph_pool_1,graph_pool_2,graph_pool_3,graph_pool_4],axis = 1)
    graph_pool = tf.keras.layers.Dense(units=8,activation='tanh',name='dense_after_pool' )(graph_pool)
    logits = tf.keras.layers.Dense(units=3,name='logits',activation = 'tanh')(graph_pool)
...
def AngularDifference(y_actual,y_pred):
    ...
    return angle
def tukey(r,c=0.5):
        return (1-(1-(r/c)**2)**3)*(c**2)/6.
def TukeyLoss(y_actual,y_pred):
    c = 1
    angle = AngularDifference(y_actual,y_pred)
    return tf.minimum(100*tukey(angle,c=c),100*c**2/6.)
...
simple_ts_model.compile(tf.keras.optimizers.Adam(learning_rate=0.001),loss=AngularDifference,metrics=metrics)
...
sim2.compile(tf.keras.optimizers.Adam(learning_rate=0.001),loss=TukeyLoss,metrics=metrics)
