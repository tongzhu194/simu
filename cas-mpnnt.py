...
def initialize_args():
    ...
    parser.add_argument(
        "-geo",
        dest="geo",#basically identify the set name
        type=str,
        required=True
    )
    parser.add_argument(
...
date = args.date
##############################
#train_ds_provider = runner.TFRecordDatasetProvider(file_pattern=f"/n/holyscratch01/arguelles_delgado_lab/Everyone/tzhu/{geo}{subgeo}{traintype}_{infile}_1_70")
train_ds_provider = runner.TFRecordDatasetProvider(file_pattern=f"/n/holyscratch01/arguelles_delgado_lab/Everyone/tzhu/E-ortho120SAM_1_50")

valid_ds_provider = runner.TFRecordDatasetProvider(file_pattern=f"/n/holyscratch01/arguelles_delgado_lab/Everyone/tzhu/E-ortho120SAM_4650_5050")
...
def complex_mpnn_model(hidden_size_emb,hidden_gcn1,hidden_gcn2,hidden_gcn3,hidden_gcn4,graph_tensor_spec):
    ...
    edgegather = edge_gatherinfo()
    midE1 = midenseEdge(hidden_size=12,activation='relu',name='mid1')
    midE2 = midenseEdge(hidden_size=36,activation='relu',name='mid2')
    midE3 = midenseEdge(hidden_size=9,activation='relu',name='mid3')
...
################################
simple_ts_model =  complex_mpnn_model(hidden_size_emb=54,hidden_gcn1=100,hidden_gcn2=60,hidden_gcn3=12,hidden_gcn4=16,graph_tensor_spec=gtspec)
...
###################
netw = 'mpnnt4'
md_path = f'./md_weights/{date}_{traintype}_{geo}{subgeo}_{netw}/'
md_path_pre = md_path+'pre'
new_model =complex_mpnn_model(hidden_size_emb=54,hidden_gcn1=100,hidden_gcn2=60,hidden_gcn3=12,hidden_gcn4=16,graph_tensor_spec=gtspec)
...
for id in range(train_Number):
    ...
    graphy = next(iterator)
    true = graphy[1]
    reco = new_model(graphy[0])
    zenithtrue.append(true)
    zenithreco.append(reco)
    Etrue.append(graphy[0].context['event_energy'])
    fID.append(graphy[0].context['event_id'])
    num_hits.append(len(graphy[0].node_sets['hit']['4Dvec']))

print("Finish train-iteration")

reco = np.array(zenithreco).flatten()
truth = np.array(zenithtrue).flatten()
Etruth = np.array(Etrue).flatten()
EID = np.array(fID).flatten()
Num = np.array(num_hits).flatten()
total = [truth,reco,Etruth,Num,EID]
np.save(f'/n/home05/tzhu/work/icgen2/simu/ortho120/{date}_{traintype}_{geo}{subgeo}_{netw}_trainset.npy',total)
print("sucessfully save train-data")
...
vfID = []
iterator = iter(valid_dataset)
print("interator built")
for id in range(valid_Number):
    print(f"valid:{id}")
    graphy = next(iterator)
    vzenithtrue.append(graphy[1])
    vzenithreco.append(new_model(graphy[0]))   
    vEtrue.append(graphy[0].context['event_energy'])
    vfID.append(graphy[0].context['event_id'])
    vnum_hits.append(len(graphy[0].node_sets['hit']['4Dvec']))


print("Finish valid-iteration")

vreco = np.array(vzenithreco).flatten()
vtruth = np.array(vzenithtrue).flatten()
vEtruth = np.array(vEtrue).flatten()
vNum = np.array(vnum_hits).flatten()
vEID = np.array(vfID).flatten()
vtotal = [vtruth,vreco,vEtruth,vNum,vEID]
np.save(f'/n/home05/tzhu/work/icgen2/simu/ortho120/{date}_{traintype}_{geo}{subgeo}_{netw}_validset.npy',vtotal)
print("sucessfully save valid-data")
